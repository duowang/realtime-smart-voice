...
Reviewed every Python source and test file, the shell runner, configuration, dependency constraints, Makefile, ignore rules, and repository documentation. Static prompt audio and the license are unchanged.

## Findings addressed

| Area | Previous problem | Result |
| --- | --- | --- |
| Conversation timeout | The timeout wrapped a wait that ran only after the conversation had already finished. | It covers greeting, connection setup, tools, and the active conversation. |
...
| Resource ownership | Shared music cleanup ran twice; prompt failures leaked streams; partial startup leaked components. | Shared resources have one owner. Cleanup attempts all components even after a failure. Prompt streams close in `finally`. |
...
Terminal album art, configuration loading, and device I/O now have focused modules. Music commands share one validation/error boundary instead of repeating broad exception handlers for every action. The greeting generator uses the same config/key loading and resolves paths from the repository.

## Validation

- `make check`: compilation, Ruff across source/tests/generator, and 89 offline tests passed on macOS, Python 3.12.
