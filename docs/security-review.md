...
- A separate Gitleaks scan of the final public-file snapshot (including new files) found **no detected secrets**. Workflow YAML, pinned action references, shell syntax, and installed dependency compatibility were checked locally; the revised GitHub workflow has not run remotely yet.
