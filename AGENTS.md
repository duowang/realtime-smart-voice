...
The assistant runtime lives in `src/`, with `realtime_voice_assistant.py` orchestrating wake-word detection, OpenAI realtime streaming, and music control via `music_commands.py`, `realtime_voice_client.py`, and `wake_word_detector.py`. `youtube_music_player.py` handles audio download/playback and cache metadata; `album_art.py` handles thumbnail caching and terminal rendering. Shared configuration and cancellation-safe device I/O live in `configuration.py` and `audio_io.py`. Configuration defaults sit in `config/config.json`, audio prompts in `audio/`, and cached media (audio + thumbnails) under `music_cache/`. Logs land in `logs/`, while downloaded sherpa-onnx wake-word models are cached in the ignored `models/` directory. Use `run.sh` for end-to-end setup, and keep large assets out of Git—store them in `audio/` or external storage.

## Build, Test, and Development Commands
- `python3.12 -m venv venv && source venv/bin/activate` — create/enter the project virtualenv (Python 3.12 is required).
- `pip install -r requirements.txt` — install runtime dependencies (OpenAI, sherpa-onnx, YouTube Music helpers, Pillow for thumbnails).
...
## Testing Guidelines
Automated tests live under `tests/` and use `pytest`. Install development dependencies with `make dev-deps`, then run `make check` for compilation, Ruff linting, and tests. Focus on deterministic components such as wake-word thresholds and music command parsing; mock audio hardware and network traffic. Include coverage notes in PRs. Smoke-test the voice loop with `./run.sh` and a short “Hi Taco” interaction before merging.
...
Store secrets in `.env` (auto-copied from `.env.example`) and never commit keys. Set `OPENAI_API_KEY` before running. Wake-word detection requires no key; `./run.sh --setup-only` downloads its model for offline reuse. Adjust model, wake-word, and audio thresholds through `config/config.json`; keep defaults conservative to prevent false activations. If you rotate keys, update the shared vault and the configuration template promptly.
